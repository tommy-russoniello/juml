...
public class ClassBoxController extends InspectorController{
	...
	ModifyText modifyName;
	ModifyText modifyAttributes;
	ModifyText modifyMethods;
	...
	/*
	 * Updates the ClassBox variables depending on the changes the user entered into the inspector 
	 * @param event is a KeyEvent that activates this method upon any key entered for real time updating
	 * @postcondition ClassBox variables are changed directly after a key is pressed so that classBox and ClassBox.fxml hold the same info
	 */
	public void updateText(KeyEvent event){
		...
		if (!name.equals(classBox.getName())) {
			modifyName.updateNewText(name);
			classBox.setName(name);
		}
		if (!attributes.equals(classBox.getAttributes())) {
			modifyAttributes.updateNewText(attributes);
			classBox.setAttributes(attributes);
		}
		if (!methods.equals(classBox.getMethods())) {
			modifyMethods.updateNewText(methods);
	...
	public void classBoxOriginChange(KeyEvent event){
		if (event.getCode() == KeyCode.ENTER){
			try{
				...
				new MoveNode(classBox, classBoxUML.getOriginX(), classBoxUML.getOriginY(),x,y,true);
	...
	public void loadInspectorInfo(UMLObject object){
		getClassBox(object);
		classBoxNameSetText();
		modifyName = new ModifyText(classBox, classBox.getNameField(), classBox.getName(), classBoxName);
		modifyAttributes = new ModifyText(classBox, classBox.getAttributesField(), classBox.getAttributes(), classBoxAttributes);
		modifyMethods = new ModifyText(classBox, classBox.getMethodsField(), classBox.getMethods(), classBoxMethods);
		classBoxAttributesSetText();
		classBoxMethodsSetText();
		classBoxOriginXSetText();
		classBoxOriginYSetText();
