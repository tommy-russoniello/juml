...
public class Relationship extends UMLConnector {
	...
	public void readInPivots(Scanner input, Controller controler) {
		...
		while (nextToken.equals("Pivot:")){
			...
			// add pivots here
			new SplitLine (this, segments.get(0), x, y, controler);
			pivots.add(new Pivot (this, x,y));
	 ...
	 * 
	 * @postcondition Positions of first and last segment (may be the same one), and
	 * notes are reset * to default (shape is resest in each individual relationship
	...
	/*
	 * Removes start and end Notes from group's children, effectively hiding them
	 * from the scene.
	 * 
	...
	/*
	 * Adds start and end Notes to group's children, effectively making them visible
	 * in the scene.
	 * 
	...
	/*
	 * Changes text of startText Note to given string if the resulting width of
	 * startText is <= 20.
	 * 
	 ...
	 * 
	 * @postcondition Text of startText Note is changed to newText unless the
	 * resulting width of * startText would be > 20 (in that case nothing is
	 * changed).
	 */
	...
	/*
	 * Changes text of endText Note to given string if the resulting width of
	 * endText is <= 20.
	 * 
	 ...
	 * 
	 * @postcondition Text of endText Note is changed to newText unless the
	 * resulting width of * endText would be > 20 (in that case nothing is changed).
	 */
	...
	public void update(double midPoint, boolean isReset) {
		...
		if (!isReset) {
			...
			// Calculate new coordinates of shape and notes and the delta of them from their
			// initial
			// * coordinates.
			endAngle = calcArcTanAngle(endLine, "end");
		...
		// If delta is NaN then reset segments, shape, and notes and move them back to
		// their initial
		// * positions.
		if (Double.isNaN(deltaEndX) || Double.isNaN(deltaEndY) || Double.isNaN(deltaStartX) || Double.isNaN(deltaStartY)
				|| isReset) {
			reset();
			...
			// Repeat above logic to move segments, notes, and shape back into place after
			// resetting.
			// ---------------------------------------------------------------------
	...
	public void update(boolean isReset) {
		// This is just so update can be called this way on any given relationship. This
		// method should
	...
	public boolean textVisible() {
		return group.getChildren().contains(startText.getModel()) || group.getChildren().contains(startText.getModel());
	}

	/*
	 * updates all segments in the Relationship.
	 * 
	 * @postcondition all segments in the Relationship update the coordinates of
	 * their lines.
	 */
